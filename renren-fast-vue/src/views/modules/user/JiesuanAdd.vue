  ...
  <el-dialog title="结算" :close-on-click-modal="false" :visible.sync="visible">
    ...
    >
      ...
      <el-form-item label="一级分享佣金" prop="jobFxbackprice">
        <el-input
          v-model="dataForm.jobFxbackprice"
          placeholder="请输入一级分享佣金"
          :disabled="id?true:false"
        >
      ...
      <el-form-item label="二级分享佣金" prop="jobFxejbackprice">
        <el-input
          v-model="dataForm.jobFxejbackprice"
          placeholder="请输入二级分享佣金"
          :disabled="id?true:false"
        >
      ...
      <el-form-item label="工资条上传" prop="paySlip" v-if="dataForm.type==1">
        ...
        >
          <img v-if="dataForm.paySlip" :src="dataForm.paySlip" class="avatar" />
      ...
      this.$nextTick(() => {
        ...
        this.$http({
          url: this.$http.adornUrl(
            `/job/sysjob/info/${this.$route.query.jobId}`
          ),
          method: "get"
        }).then(({ data }) => {
          if (data && data.code === 0) {
        ...
        this.$http({
          url: this.$http.adornUrl(
            `/app/myrelation/list?openid=${this.$route.query.openid}`
          ),
          method: "get"
        }).then(({ data }) => {
          if (data && data.code === 0) {
            ...
            this.firstFirends = arr;
          }
        });
        /* 获取二级好友 */
        this.$http({
          url: this.$http.adornUrl(
            `/app/myrelation/nextlist?openid=${this.$route.query.openid}`
          ),
          method: "get"
        }).then(({ data }) => {
          if (data && data.code === 0) {
            ...
            that
              .$http({
                url: that.$http.adornUrl(
                  that.id ? "/my/mypaystubs/update" : `/my/mypaystubs/save`
                ),
                method: "post",
                data: that.$http.adornData({
                  ...
                  payForTime: moment(that.dataForm.payForTime).format(
                    "YYYY-MM"
                  ),
